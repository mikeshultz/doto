...
function AddCalendarModal(props) {
  ...
  const [googleAuth, { called, error: mutationError, loading, data: mutationData }] = useMutation(GOOGLE_AUTH)
  ...
  return (
    <div className="modal">
      <form onSubmit={save}>
        ...
        <Field name="calendarid" label="Calendar ID (E-mail)" onChange={onCalendarIDChange} />
        <div className="button-group">
          <button type="submit" className="save">Save</button>
          <button className="close" onClick={() => { setCalendarId(''); setModalState(-1) }}>Close</button>
        </div>
      </form>
    </div>
  )
